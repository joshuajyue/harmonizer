...
async function bootstrap() {
  ...
  try {
    ...
    current.setSlotLoading("A");
    current.setSlotLoading("B");
    ...
    latest.setSlotResult("A", resultA, current.melodyRevision);
    latest.setSlotResult("B", resultB, current.melodyRevision);
    latest.setViewMode("A");
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Could not load engines.";
    const current = useStudioStore.getState();
    if (current.engines.length === 0) current.setEnginesError(message);
